...
async function fetchEggByCode(code: string): Promise<Egg> {
...
export default function EasterEggByCodePage() {
    ...
    const title = 'Gratulacje! Znalazłeś fizycznego easter egga.'
    return (
        <div className="mx-auto flex max-w-md flex-col gap-4 pt-20 p-6">
            <h1 className="text-2xl font-bold">{title}</h1>
            <div className="rounded-lg border bg-white/5 p-4">
                <p className="text-sm text-gray-300">CODE: <span className="font-mono">{egg.code}</span></p>
                ...
                {!egg.active && (
                    <p className="mt-2 text-sm text-red-400">
                        Ten easter egg jest już nieaktywny
                        {egg.claimedBy && <> — odebrał(a): <b>{egg.claimedBy.name}</b></>}.</p>
                )}
            </div>

            {egg.active ? (
                <div className="rounded-lg border bg-white/5 p-4 flex flex-col gap-3">
                    <p className="text-sm">To jajko jest warte <strong>50 $pruch</strong>. Wybierz uczestnika, aby odebrać punkty.</p>
                    <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
                        <Select onValueChange={setSelected} value={selected || undefined}>
                            <SelectTrigger className="w-64"><SelectValue placeholder="Wybierz uczestnika" /></SelectTrigger>
                            <SelectContent>
                    ...
                    <p className="text-xs text-gray-400">Po odebraniu to jajko zostanie oznaczone jako nieaktywne.</p>
                </div>
            ) : (
                isAdmin && (
                    <div className="rounded-lg border bg-white/5 p-4 flex items-center gap-2">
                        <Button variant="secondary" onClick={() => reactivateMut.mutate()} disabled={reactivateMut.isPending}>
                            {reactivateMut.isPending ? 'Przywracanie…' : 'Przywróć do gry (admin)'}
                        </Button>
                        <p className="text-xs text-gray-400">Zazwyczaj przywracamy tylko fizyczne jajka.</p>
                    </div>
                )
            )}

            <div className="rounded-lg border bg-white/5 p-4 text-xs text-gray-400">
                {egg.label && <p><span className="text-gray-500">Etykieta:</span> <span className="text-gray-300">{egg.label}</span></p>}
